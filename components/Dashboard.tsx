...
const Dashboard: React.FC<Props> = ({ onOpenProject, onShowOnboarding, onShowModelConfig }) => {
  ...
  const confirmDelete = async (e: React.MouseEvent, id: string) => {
    ...
    try {
        ...
        showAlert(`鍒犻櫎椤圭洰澶辫触: ${error instanceof Error ? error.message : '鏈煡閿欒'}`, { type: 'error' });
    } finally {
        setDeleteConfirmId(null);
    }
  };
    ...
    showAlert('确定从资产库删除该资源吗？', {
      ...
      onConfirm: async () => {
        try {
          ...
          showAlert(`鍒犻櫎璧勪骇澶辫触: ${error instanceof Error ? error.message : '鏈煡閿欒'}`, { type: 'error' });
        }
      }
    });
  };
        ...
        <header className="mb-16 border-b border-[var(--border-subtle)] pb-8 flex items-end justify-between">
          <div>
            <h1 className="text-3xl font-light text-[var(--text-primary)] tracking-tight mb-2 flex items-center gap-3">
              椤圭洰搴?              <span className="text-[var(--text-muted)] text-lg">/</span>
          ...
          <div className="flex items-center gap-3">
            ...
            {onShowOnboarding && (
              ...
              >
                ...
                <span className="font-medium text-xs tracking-widest uppercase">甯姪</span>
              </button>
            )}
            <button
            ...
            >
              ...
              <span className="font-medium text-xs tracking-widest uppercase">绯荤粺璁剧疆</span>
            </button>
            <button
              onClick={toggleTheme}
              className="group flex items-center gap-2 px-4 py-3 border border-[var(--border-primary)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:border-[var(--border-secondary)] transition-colors"
              title={theme === 'dark' ? '鍒囨崲浜壊涓婚' : '鍒囨崲鏆楄壊涓婚'}
            >
              {theme === 'dark' ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
              <span className="font-medium text-xs tracking-widest uppercase">{theme === 'dark' ? '浜壊' : '鏆楄壊'}</span>
            </button>
            <button 
            ...
            >
              ...
              <span className="font-bold text-xs tracking-widest uppercase">鏂板缓椤圭洰</span>
            </button>
          </div>
        </header>
                  ...
                  {deleteConfirmId === proj.id && (
                    <div className="absolute inset-0 z-20 bg-[var(--bg-primary)] flex flex-col items-center justify-center p-6 space-y-4 animate-in fade-in duration-200" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex gap-2 w-full pt-2">
                            <button onClick={cancelDelete} className="flex-1 py-3 bg-[var(--bg-surface)] hover:bg-[var(--bg-hover)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] text-[10px] font-bold uppercase tracking-wider transition-colors border border-[var(--border-primary)]">鍙栨秷</button>
                            <button onClick={(e) => confirmDelete(e, proj.id)} className="flex-1 py-3 bg-[var(--error-hover-bg)] text-[var(--error-text)] text-[10px] font-bold uppercase tracking-wider transition-colors border border-[var(--error-border)]">姘镐箙鍒犻櫎</button>
                        </div>
                    </div>
                  )}

                  <div className="flex-1 p-6 relative flex flex-col">
                     <button onClick={(e) => requestDelete(e, proj.id)} className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 p-2 hover:bg-[var(--bg-hover)] text-[var(--text-muted)] hover:text-[var(--error-text)] transition-all rounded-sm z-10" title="鍒犻櫎椤圭洰">
                     ...
                     <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                            <span className="text-[9px] font-mono text-[var(--text-tertiary)] border border-[var(--border-primary)] px-1.5 py-0.5 uppercase tracking-wider">
                              <Users className="w-3 h-3 inline mr-1" />{proj.characterLibrary?.length || 0} 瑙掕壊
                            </span>
                            <span className="text-[9px] font-mono text-[var(--text-tertiary)] border border-[var(--border-primary)] px-1.5 py-0.5 uppercase tracking-wider">
                              <Film className="w-3 h-3 inline mr-1" />澶氬墽闆?                            </span>
                        </div>
                        {proj.description && (
          ...
          >
            ...
            <div className="space-y-4 text-center">
              ...
              <div className="inline-block">
                <img src={qrCodeImg} alt="浜ゆ祦缇や簩缁寸爜" className="w-64 h-64 object-contain" />
              </div>
              <div className="text-[10px] text-[var(--text-muted)] font-mono">浜岀淮鐮佹湁鏁堟湡璇蜂互瀹為檯涓哄噯</div>
            </div>
          </div>
        </div>
      )}
          ...
          >
            ...
            <div className="flex items-end justify-between border-b border-[var(--border-subtle)] pb-4 mb-6">
              <div>
                ...
                <p className="text-xs text-[var(--text-tertiary)] mt-2">绠＄悊妯″瀷閰嶇疆銆佽祫浜у簱浠ュ強鏁版嵁瀵煎叆瀵煎嚭</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {onShowModelConfig && (
                ...
                >
                  ...
                  <div className="text-[10px] text-[var(--text-tertiary)] font-mono mt-2">绠＄悊妯″瀷涓?API 璁剧疆</div>
                </button>
              )}
              ...
              >
                <div className="flex items-center gap-2 text-[var(--text-primary)] text-sm font-bold">
                  ...
                  璧勪骇搴?                </div>
                <div className="text-[10px] text-[var(--text-tertiary)] font-mono mt-2">娴忚骞跺鐢ㄨ鑹蹭笌鍦烘櫙璧勪骇</div>
              </button>
              ...
              >
                ...
                <div className="text-[10px] text-[var(--text-tertiary)] font-mono mt-2">瀵煎嚭鍏ㄩ儴椤圭洰涓庤祫浜у簱澶囦唤</div>
              </button>
              ...
              >
                ...
                <div className="text-[10px] text-[var(--text-tertiary)] font-mono mt-2">瀵煎叆鍏ㄩ儴椤圭洰涓庤祫浜у簱澶囦唤</div>
              </button>
            </div>
          </div>
        </div>
          ...
          >
            ...
            <div className="flex items-end justify-between border-b border-[var(--border-subtle)] pb-6 mb-6">
              <div>
                <h2 className="text-lg text-[var(--text-primary)] flex items-center gap-2">
                  <Archive className="w-4 h-4 text-[var(--accent-text)]" />
                  璧勪骇搴?                  <span className="text-[var(--text-muted)] text-xs font-mono uppercase tracking-widest">Asset Library</span>
                </h2>
                <p className="text-xs text-[var(--text-tertiary)] mt-2">
                  鍦ㄩ」鐩噷灏嗚鑹蹭笌鍦烘櫙鍔犲叆璧勪骇搴擄紝璺ㄩ」鐩鐢?                </p>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              ...
              <div className="flex gap-2">
                {(['all', 'character', 'scene', 'prop'] as const).map((type) => (
                  ...
                  >
                    {type === 'all' ? '鍏ㄩ儴' : type === 'character' ? '瑙掕壊' : type === 'scene' ? '鍦烘櫙' : '閬撳叿'}
                  </button>
                ))}
              </div>
            </div>
            ...
            {isLibraryLoading ? (
              ...
              <div className="border border-dashed border-[var(--border-primary)] rounded-xl p-10 text-center text-[var(--text-muted)] text-sm">
                鏆傛棤璧勪骇銆傚彲鍦ㄩ」鐩殑鈥滆鑹蹭笌鍦烘櫙鈥濅腑鍔犲叆璧勪骇搴撱€?              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                      ...
                      <div className="p-4 space-y-3">
                        <div>
                          ...
                          <div className="text-[10px] text-[var(--text-tertiary)] font-mono uppercase tracking-widest mt-1">
                            {item.type === 'character' ? '瑙掕壊' : item.type === 'scene' ? '鍦烘櫙' : '閬撳叿'}
                          </div>
                          <div className="text-[10px] text-[var(--text-muted)] font-mono mt-1 line-clamp-1">
                            {(item.projectName && item.projectName.trim()) || '鏈煡椤圭洰'}
                          </div>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[var(--bg-base)]/70 p-6" onClick={() => setAssetToUse(null)}>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {projects.length === 0 ? (
                <div className="text-[var(--text-muted)] text-sm">鏆傛棤椤圭洰鍙敤</div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {projects.map((proj) => (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-[var(--text-tertiary)] font-mono mt-1">鏈€鍚庝慨鏀? {formatDate(proj.lastModified)}</div>
                    </button>
                  ))}
                </div>
              )}
